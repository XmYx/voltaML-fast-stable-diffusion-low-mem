...
import ray
#num_cpus = 8 # or whatever.. basically if you don't set this then ray will create total number of cpus on your computer as idle processes waiting to get used
try:
    ray.shutdown()
except:
    pass
ray.init(num_gpus=1)#, num_cpus=num_cpus)
...
@app.post('/api/infer')
async def infer(body: InferenceArgs):
    #print(body)
    #global trt
    ...
    response = await asyncio.gather(worker.generate.remote(body, init_image))
