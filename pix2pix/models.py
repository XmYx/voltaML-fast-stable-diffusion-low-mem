...
class Optimizer:
    ...
    def remove_casts(self):
        ...
        for node in self.graph.nodes:
            ...
            if (
                node.op in ["Add", "Transpose"]
                and len(node.outputs[0].outputs) == 3
                and node.o().op == "Cast"
                and node.o(1).op == "Cast"
                and node.o(2).op == "Cast"
            ):
                ...
                for nextNode in node.outputs[0].outputs:
                    if nextNode.op == "Mul":
                        if swishOutputTensor is None:
                            ...
                            assert (
                                nextGemmNode.op == "Gemm"
                            ), "Unexpected node type for nextGemmNode {}".format(
                                nextGemmNode.name
                            )
                            nextGemmNode.inputs = [
                                swishOutputTensor,
                                nextGemmNode.inputs[1],
                                nextGemmNode.inputs[2],
                            ]
    ...
    def insert_splitgelu_plugin(self):
        ...
        for node in self.graph.nodes:
            if node.op == "Erf":
                ...
                splitGeLUV = gs.Variable(
                    "splitGeLUV-" + str(nSplitGeLUPlugin),
                    np.dtype(np.float32),
                    outputShape,
                )
                splitGeLUN = gs.Node(
                    "SplitGeLU",
                    "splitGeLUN-" + str(nSplitGeLUPlugin),
                    inputs=[inputTensor],
                    outputs=[splitGeLUV],
                )
    ...
    def insert_seq2spatial_plugin(self):
        ...
        for node in self.graph.nodes:
            if node.op == "Transpose" and node.o().op == "Conv":
                ...
                reshapeNode = node.i()
                assert (
                    reshapeNode.op == "Reshape"
                ), "Unexpected node type for reshapeNode {}".format(reshapeNode.name)
                residualNode = reshapeNode.i(0)
                assert (
                    residualNode.op == "Add"
                ), "Unexpected node type for residualNode {}".format(residualNode.name)
                biasNode = residualNode.i(0)
                assert (
                    biasNode.op == "Add"
                ), "Unexpected node type for biasNode {}".format(biasNode.name)
                biasIndex = [
                    type(i) == gs.ir.tensor.Constant for i in biasNode.inputs
                ].index(True)
                bias = np.array(
                    deepcopy(biasNode.inputs[biasIndex].values.tolist()),
                    dtype=np.float32,
                )
                biasInput = gs.Constant(
                    "AddAddSeqLen2SpatialBias-" + str(nSeqLen2SpatialPlugin),
                ...
                seqLen2SpatialNode = gs.Node(
                    "SeqLen2Spatial",
                    "AddAddSeqLen2Spatial-" + str(nSeqLen2SpatialPlugin),
                    attrs=OrderedDict(
                        [
                            ("height", outputTensor.shape[2]),
                            ("width", outputTensor.shape[3]),
                        ]
                    ),
                    inputs=[inputTensor, biasInput, residualInput],
...
class CLIP(BaseModel):
    def get_model(self):
        return CLIPTextModel.from_pretrained("openai/clip-vit-large-patch14").to(
            self.device
        )
    ...
    def optimize(self, onnx_graph, minimal_optimization=False):
        ...
        if bLayerNormPlugin:
            ...
            opt.info(
                "CLIP: inserted " + str(num_layernorm_inserted) + " LayerNorm plugins"
            )
...
class UNet(BaseModel):
    ...
    def get_input_profile(
        ...
        return {
            "sample": [
                (3 * min_batch, 8, min_latent_height, min_latent_width),
                (3 * batch_size, 8, latent_height, latent_width),
                (3 * max_batch, 8, max_latent_height, max_latent_width),
            ],
            "encoder_hidden_states": [
                (3 * min_batch, self.text_maxlen, self.embedding_dim),
    ...
    def optimize(self, onnx_graph, minimal_optimization=False):
        ...
        if bRemoveInstanceNorm:
            ...
            opt.info(
                "UNet: replaced " + str(num_instancenorm_replaced) + " InstanceNorms"
            )
        ...
        if bRemoveParallelSwish:
            ...
            opt.info(
                "UNet: removed "
                + str(num_parallel_swish_removed)
                + " parallel swish ops"
            )
        ...
        if bGroupNormPlugin and not bDisablePlugins:
            ...
            opt.info(
                "UNet: inserted " + str(num_groupnorm_inserted) + " GroupNorm plugins"
            )

        if bLayerNormPlugin and not bDisablePlugins:
            num_layernorm_inserted = opt.insert_layernorm_plugin()
            opt.info(
                "UNet: inserted " + str(num_layernorm_inserted) + " LayerNorm plugins"
            )

        if bSplitGeLUPlugin and not bDisablePlugins:
            num_splitgelu_inserted = opt.insert_splitgelu_plugin()
            opt.info(
                "UNet: inserted " + str(num_splitgelu_inserted) + " SplitGeLU plugins"
            )
...
class VAE(BaseModel):
    def get_model(self):
        vae = AutoencoderKL.from_pretrained(
            self.model_path, subfolder="vae", use_auth_token=self.hf_token
        ).to(self.device)
    ...
    def get_input_profile(
        ...
        return {
            "latent": [
                (3, 8, min_latent_height, min_latent_width),
                (3, 8, latent_height, latent_width),
                (3, 8, max_latent_height, max_latent_width),
            ]
        }
    ...
    def optimize(self, onnx_graph, minimal_optimization=False):
        ...
        if bRemoveInstanceNorm:
            num_instancenorm_replaced = opt.decompose_instancenorms()
            opt.info(
                "VAE: replaced " + str(num_instancenorm_replaced) + " InstanceNorms"
            )
        ...
        if bGroupNormPlugin:
            num_groupnorm_inserted = opt.insert_groupnorm_plugin()
            opt.info(
                "VAE: inserted " + str(num_groupnorm_inserted) + " GroupNorm plugins"
            )
