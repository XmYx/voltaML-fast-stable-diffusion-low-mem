...
class MainWindow(QWidget):
    ...
    def on_infer_button_clicked(self):
        ...
        if self.lastinit != None:
            ...
            frame = cv2.addWeighted(frame, 0.5, np.array(self.lastinit), 0.20, 0)
    ...
    def handle_response(self, reply):
        ...
        result = unpackb(data)
        #print(result['images'])
        result['images'] = [Image.open(io.BytesIO(result['images']))]
        self.lastinit = result['images'][0].convert('RGB')
        #print(result['images'])
        ...
        self.img = QPixmap.fromImage(QImage(ImageQt(result['images'][0])))
