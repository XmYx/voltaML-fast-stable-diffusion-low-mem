...
class Engine:
    ...
    def build(self, onnx_path, fp16, input_profile=None, enable_preview=False):
        ...
        if "unet" in onnx_path:
            p = None
            p = Profile()
            p.add('sample', min=(2, 8, 64, 64), opt=(2, 8, 64, 64), max=(2, 8, 64, 64)).add('encoder_hidden_states',min=(2, 77, 768),opt=(2, 77, 768),max=(2, 77, 768)).add('timestep', min=[1], opt=[1], max=[1])
...
def save_image(
    images, image_path_dir: str = "", image_name_prefix: str = "", save: bool = True
):
    """
    Convert a tensor to a PIL image and save it to a given path.
    """
    imgs = []
    images = (
        ((images + 1) * 255 / 2)
    ...
    for i in range(images.shape[0]):
        if save:
            ...
            image_path = f"{image_path_dir}/{str(random.randint(1000, 9999))}.png"
